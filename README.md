...
本项目使用的数据集包含淘宝用户的历史行为数据，关键字段如下：

| 字段名                  | 说明                           | 示例           |
| :---------------------- | :----------------------------- | :------------- |
| **user_id**       | 用户唯一标识符                 | `100`        |
| **item_id**       | 商品唯一标识符                 | `200`        |
| **item_category** | 商品所属类别                   | `服装`       |
| **behavior_type** | 用户行为类型 (pv/fav/cart/buy) | `pv` (浏览)  |
| **timestamp**     | 行为发生的时间戳               | `1511544070` |
| **price**         | 商品价格                       | `199.00`     |

*(注：数据文件默认位于 `data/UserBehavior.csv`)*
